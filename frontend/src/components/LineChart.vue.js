...
if (__VLS_ctx.series.some((item) => item.values.length)) {
    __VLS_asFunctionalElement1(__VLS_intrinsics.div, __VLS_intrinsics.div)({
        ...{ class: "chart-legend" },
    });
...
for (const [value, index] of __VLS_vFor((__VLS_ctx.leftGridValues))) {
    ...
    // @ts-ignore
    [width, width, width, height, leftGridValues, padding, padding, padding, padding, padding, padding, padding, padding, plotHeight, plotHeight, plotHeight, plotHeight, compact, compact, valueSuffix, hasRightAxis, rightGridValues,];
}
